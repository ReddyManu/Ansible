...
CREATE() {
  ...
  if [ $COUNT -eq 0 ]; then
    aws ec2 run-instances --image-id ami-0e4e4b2f188e91845 --instance-type t2.micro --security-group-ids sg-0f65da430b2ca3fb9 --tag-specifications "ResourceType=instance,Tags=[{Key=Name,Value=$1}]" | jq &>/dev/null
  else
